...
function SideBar({ sideBarActive, setSideBarActive }: Props) {
  ...
  const sideBarShowClass =
    "fixed -left-1/2 top-0 h-full w-1/2 translate-x-full bg-white duration-700 ease-out z-20";
  const sideBarHideClass =
    "fixed -left-1/2 top-0 h-full w-1/2 -translate-x-full bg-white duration-700 ease-in z-20";
  const backdropActiveClass =
    "fixed left-0 top-0 z-10 h-full w-full bg-zinc-500 opacity-50 transition-opacity duration-1000";
  const backdrophidden =
    "fixed left-0 top-0 -z-10 h-full w-full bg-zinc-500 opacity-0 transition-opacity duration-1000";
  return (
    <>
      <div className={sideBarActive ? sideBarShowClass : sideBarHideClass}>
        <div className="relative grid h-1/2 content-center justify-center">
